...
pub fn parse_cli_args(args: &[String]) -> Result<CliCommand, BitcoinError> {
    match args.get(0).map(String::as_str) {
        Some("send") => {
            let amount_str = args
                .get(1)
                .ok_or_else(|| BitcoinError::ParseError("missing amount argument".to_string()))?;

            let amount = u64::from_str(amount_str).map_err(|_| BitcoinError::InvalidAmount)?;

            let address = args
                .get(2)
                .ok_or_else(|| BitcoinError::ParseError("missing address argument".to_string()))?
                .clone();
